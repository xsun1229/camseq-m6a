---
title: "Comparing m6a sites from sample2-4 (merged and separate) with camseq paper"
author: "XSun"
date: "2026-02-03"
output:
...
make_file_sample2 <- function(rd, p_cutoff_str) {
  sprintf(
    "/project/xinhe/xsun/camseq/pipeline/workspace_round3/count_sites/pileup/processed_binomp_%s_1n/sample02_with_id_read%d_binomp%s.tsv",
...
| sample     | pvalue  | read depth | methylation level | #of m6asites | overlap with paper1 (m6a sites from HSPC ribo– d9)  | overlap with paper 2 (REPIC m6a peaks) |
|------------|---------|------------|-------------------|--------------|-----------------------------------------------------|----------------------------------------|
| merged 2-4 | < 0.001 | >= 20      | >= 0.05           | 49346        | 8.60%                                               | 35.60%                                 |
| merged 2-4 | < 0.001 | >= 20      | >= 0.1            | 35688        | 12.10%                                              | 41.71%                                 |
| merged 2-4 | < 0.001 | >= 20      | >= 0.2            | 21198        | 17.30%                                              | 52.30%                                 |
| sample 2   | < 0.001 | >= 20      | >= 0.05           | 5072         | 15.60%                                              | 29.00%                                 |
| sample 2   | < 0.001 | >= 20      | >= 0.1            | 3989         | 18.90%                                              | 31.40%                                 |
| sample 2   | < 0.001 | >= 20      | >= 0.2            | 2428         | 27.30%                                              | 38.90%                                 |

# Merged Sample 2-4, cutoffs for our data: p<0.001, read depth >=20, methylation level >= 0.05
...
file_ours <- make_file_sample234(rd, pc)
...
DT::datatable(head(df_paper_raw),caption = htmltools::tags$caption( style = 'caption-side: left; text-align: left; color:black;  font-size:150% ;','Several sites reported by the paper'),options = list(pageLength = 10) )
...
We check the coverage of our sites by the m6a peaks identified in MONO-MAC-6.
...
DT::datatable(head(df_paper_raw),caption = htmltools::tags$caption( style = 'caption-side: left; text-align: left; color:black;  font-size:150% ;','Several sites reported by REPIC'),options = list(pageLength = 10) )

print(paste0("Total number of m6a peaks reported by the paper ", nrow(df_paper_raw)))
...
DT::datatable(head(df_paper_raw),caption = htmltools::tags$caption( style = 'caption-side: left; text-align: left; color:black;  font-size:150% ;','Several sites reported by m6a atlas'),options = list(pageLength = 10) )
...
# Merged Sample 2-4, cutoffs for our data: p<0.001, read depth >=20, methylation level >= 0.1
...
# Merged Sample 2-4, cutoffs for our data: p<0.001, read depth >=20, methylation level >= 0.2
...
# Sample 2 only, cutoffs for our data: p<0.001, read depth >=20, methylation level >= 0.05

our merged data has LPS treatment, which probably add more m6A sites. So we limit to sample 2 only (NI)
...
file_ours <- make_file_sample2(rd, pc)
df_ours_raw <- fread(file_ours)
...
# Sample 2 only, cutoffs for our data: p<0.001, read depth >=20, methylation level >= 0.1
...
# Sample 2 only, cutoffs for our data: p<0.001, read depth >=20, methylation level >= 0.2
